...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            if df.empty:
                return "No data found for the entered stock symbol. Please try again."
        except Exception as e:
            return f"An error occurred while downloading stock data: {str(e)}"
        ...
        data_training_array = scaler.fit_transform(data_training.values.reshape(-1, 1))  # Reshape to 2D
        
        # Prepare data for prediction
        past_100_days = data_training.tail(100)
        final_df = pd.concat([past_100_days, data_testing], ignore_index=True)
        input_data = scaler.transform(final_df.values.reshape(-1, 1))  # Reshape to 2D
        ...
        # Reshape x_test for LSTM model (samples, timesteps, features)
        x_test = x_test.reshape((x_test.shape[0], x_test.shape[1], 1))

        # Make predictions
        y_predicted = model.predict(x_test)
        
        # Reshape y_predicted to 2D for inverse scaling
        y_predicted = y_predicted.reshape(-1, 1)  # From (samples, 1, 1) or (samples, 1) to (samples, 1)
